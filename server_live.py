...
async def persist_chunk_model(chunk_model: ChunkStructuredOutput):
    data = chunk_model.model_dump()

    # Append JSON + update session file
    await asyncio.to_thread(_append_jsonl_sync, data)
    SESSION_AGG["chunks"].append(data)
    await asyncio.to_thread(_write_json_atomic_sync, JSON_PATH, SESSION_AGG)

    # Insert into Chroma
    try:
        add_json_to_chroma(data)
    except Exception as e:
...
def search_events(query: str, event_type: str = "all", n_results: int = 5):
    collections = {
        ...
        "player": client.get_collection("character_events"),
    }
