# server_live.py — Live STT + AutoGen(Ollama) summarizer (await-safe)
# Uses YOUR schema:
#   world_state_updates, character_events, quest_updates
#
# Run Ollama, pull a model, then start:
#   ollama serve
#   ollama pull gemma3:12b     # or gemma3:12b / llama3.1 etc.
#   OLLAMA_MODEL=gemma3:12b python server_live.py

from __future__ import annotations
import os, re, json, asyncio, inspect
...
def initial_prompt() -> str:
    # Bias names and keep numbers as digits
...
async def summarize_with_autogen(text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Some versions are async coroutines; others are sync:
...
def summarize_rule_based_sync(text: str) -> Dict[str, Any]:
    ...
    # If nothing matched, compress narration as world update
...
async def summarize_original_style(text: str) -> Dict[str, Any]:
    # 1) AutoGen/Ollama (await if needed)
    ...
    try:
        while True:
            ...
            for utt in utterances:
                ...
                # --- Broadcast mapped to your UI expectations ---
                # World state
                ...
                # Character events -> Action + Outcome
                for item in data.get("character_events", []):
                    who = item.get("character") or "Unknown"
                ...
                # Quests
                for item in data.get("quest_updates", []):
